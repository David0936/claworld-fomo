...
| 趋势退出 | 连续两个定时确认轮次收低 | 视为上升趋势未恢复，模拟清仓并归档 |
...
| 模型2 · 回本滚仓 | 2倍时费后净收回100U；4倍、8倍、16倍及后续每次翻倍卖出剩余仓位的一半 |
| 模型3A · 2.5倍滚动 | 首次观察到2.5倍时净收回100U，其余继续观察 |
| 模型3B · 2.5倍滚动 | 首次观察到2.5倍时净收回110U，其余继续观察 |

任何模型只使用当时已经记录的数据，不用之后行情倒填此前未观察到的成交。Fomo 占比确认低于15%、连续两个定时确认轮次收低且趋势没有恢复，或连续3轮不符合清单资格时，按规则退出。单次20%或50%回撤本身不再触发清仓。

新候选按“洗盘后回升”确认：第一次合格只保存候选；下一独立扫描需要出现回踩；再下一轮重新上涨、仍满足市值/年龄/Fomo条件，且没有超过首次候选价10%，才登记模拟买入。没有形成“回踩→回升”结构就不进入正式监测。这是入场质量过滤，无法证明或预测市场最高点。
