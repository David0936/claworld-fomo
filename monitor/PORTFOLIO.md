...
全部已登记标的可通过本机 GET http://127.0.0.1:8765/api/portfolio 读取；或在monitor目录调用portfolio.report()。相对首次合格价下跌达到40%的标的立即记终值、模拟清算并转入“过往监测”，不等待3轮；全部样本、账本和曲线继续保留。模型1独立退出不影响其他模型仍未清仓的价格采样。
...
最新首次命中的标的在主清单高亮为“最新入选 · 当前规则观察点”，只表示它是最新通过规则的观测，不是确定买入建议。首次 `watch_eligible:false` 标记“可能移出 1/3轮”；连续第二轮仍不符合显示 2/3轮；连续第三轮仍不符合后转入“过往监测”。任意一轮重新符合会把计数清零。价格跌幅达到40%则立即转入，不使用3轮等待。转入只停止主清单展示与重点筛选，不删除 SQLite 样本、事件或 Markdown 档案。
