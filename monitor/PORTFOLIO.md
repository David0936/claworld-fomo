...
全部已登记标的可通过本机 GET http://127.0.0.1:8765/api/portfolio 读取；或在monitor目录调用portfolio.report()。不再设置固定20%或50%价格止损。连续两个有效定时确认轮次相对前一确认价收低，表示趋势破坏后仍未恢复，按第二个确认价模拟清仓并转入“过往监测”；全部样本、账本和曲线继续保留。已归档的旧止损记录保持原样，不因规则更新重新进入主清单。
...
新候选的第一次合格观测只进入待确认。下一独立扫描必须仍合格且价格低于或等于首次候选价，记录为洗盘回踩；再下一独立扫描必须仍合格、价格高于回踩价且不超过首次候选价的110%，才以该回升价登记基线并正式推送。任一阶段条件失效则放弃或重新开始候选观察。本规则用于减少追高，不代表能确认真正最高点。候选状态保存在自动化任务上下文和本机数据目录，不能用同轮重复查询完成确认。

最新首次命中的标的在主清单高亮为“最新入选 · 当前规则观察点”，只表示它是最新通过洗盘回升确认的观测，不是确定买入建议。首次 `watch_eligible:false` 标记“可能移出 1/3轮”；连续第二轮仍不符合显示 2/3轮；连续第三轮仍不符合后转入“过往监测”。任意一轮重新符合会把计数清零。转入只停止主清单展示与重点筛选，不删除 SQLite 样本、事件或 Markdown 档案。
